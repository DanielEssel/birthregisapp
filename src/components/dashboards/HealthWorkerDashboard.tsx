...
import { 
  Plus, Search, Filter, Baby, Clock, CheckCircle, AlertTriangle, Users, 
  MapPin, Phone, Calendar, Eye, TrendingUp, Wifi, WifiOff, FileText, Heart,
...
import { BirthRecord, BirthCaptureFormData, HealthIntegrationFormData } from '../../types';
import { mockBirthRecords, getStatusColor, getStatusText, getSyncStatusColor, getSyncStatusText } from '../../data/mockData';

export default function HealthWorkerDashboard() {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [showBirthCapture, setShowBirthCapture] = useState(false);
  const [showHealthIntegration, setShowHealthIntegration] = useState<BirthRecord | null>(null);
  const [selectedRecord, setSelectedRecord] = useState<BirthRecord | null>(null);
  const [birthRecords, setBirthRecords] = useState<BirthRecord[]>(mockBirthRecords);
  ...
  const filteredRecords = birthRecords.filter(record => {
    const matchesSearch = 
      ...
      (record.provisionalId && record.provisionalId.toLowerCase().includes(searchTerm.toLowerCase()));
    const matchesStatus = statusFilter === 'all' || record.status === statusFilter;
    return matchesSearch && matchesStatus;
  });

  const pendingSyncCount = birthRecords.filter(r => r.syncStatus === 'local').length;

  const handleBirthCapture = (formData: BirthCaptureFormData, saveLocal = false) => {
    const provisionalId = `PROV-${new Date().getFullYear()}-${String(Date.now()).slice(-6)}`;
    const healthRecordId = `HR-${new Date().getFullYear()}-${String(Date.now()).slice(-6)}`;
    ...
    const newRecord: BirthRecord = {
      ...
      placeOfBirth: mockHospitals.find(h => h.id === formData.hospitalId)?.name || '',
      ...
      lastSyncDate: saveLocal || isOffline ? undefined : new Date().toISOString(),
  ...
  const handleHealthIntegration = (record: BirthRecord, formData: HealthIntegrationFormData) => {
  ...
  const handleSync = async () => {
    ...
    setBirthRecords(prev => prev.map(r => 
      r.syncStatus === 'local' 
        ? { ...r, syncStatus: 'synced', lastSyncDate: new Date().toISOString() }
        : r
    ));
  };

  return (
    <Layout title="Health Worker Dashboard" subtitle="Birth registration at point of delivery">
      <div className="space-y-8">
        {/* Header with Offline Indicator */}
        <div className="bg-gradient-to-r from-teal-600 to-blue-600 rounded-2xl p-6 text-white">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold mb-2">Birth Registration System</h1>
              <p className="text-teal-100">Immediate capture and progressive completion</p>
            </div>
            <div className="flex items-center space-x-4">
              <OfflineIndicator pendingSyncCount={pendingSyncCount} onSync={handleSync} />
              <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-2xl font-bold text-gray-900">
                  {birthRecords.filter(r => r.stage === 'immediate_capture').length}
                </p>
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-2xl font-bold text-gray-900">
                  {birthRecords.filter(r => r.stage === 'pre_registration').length}
                </p>
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-2xl font-bold text-gray-900">
                  {birthRecords.filter(r => r.stage === 'health_integration').length}
                </p>
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-2xl font-bold text-gray-900">{pendingSyncCount}</p>
        ...
        <div className="space-y-4">
          ...
          {filteredRecords.length === 0 ? (
            <div className="bg-white rounded-xl p-8 shadow-sm border text-center">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">No birth records found</h3>
                    ...
                    <div className="flex-1">
                      <div className="flex items-start justify-between mb-4">
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <span className={`inline-flex items-center px-2 py-1 rounded text-xs font-medium ${getSyncStatusColor(record.syncStatus)}`}>
                      ...
                      <div className="mb-4">
                        ...
                        <div className="flex justify-between mt-2 text-xs text-gray-500">
                          <span className={record.currentStep >= 1 ? 'text-teal-600 font-medium' : ''}>
                            Immediate Capture
                          </span>
                          <span className={record.currentStep >= 2 ? 'text-teal-600 font-medium' : ''}>
                            Pre-Registration
                          </span>
                          <span className={record.currentStep >= 3 ? 'text-teal-600 font-medium' : ''}>
                            Health Integration
                          </span>
                          <span className={record.currentStep >= 4 ? 'text-teal-600 font-medium' : ''}>
                            Civil Registration
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="text-sm text-gray-500">
                          <div className="flex items-center space-x-4">
                            <span>Captured: {new Date(record.birthCapturedDate).toLocaleDateString()}</span>
                      ...
                      {record.requiresInfo && record.requiresInfo.length > 0 && (
                        <div className="mt-4 p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                          <p className="text-sm text-yellow-800">
                            ...
                            {record.requiresInfo.map(info => info.replace('_', ' ')).join(', ')}
                          </p>
                        </div>
                      )}
                    </div>
        ...
        {showHealthIntegration && (
          <HealthIntegrationForm
            ...
            onSubmit={(data) => handleHealthIntegration(showHealthIntegration, data)}
          />
        )}
